...
/**
 * 测试失误关闭运行中的流程，然后手动改数据恢复
 */
@Slf4j
public class FlowableRecoverTest {
    ...
    private String statusKey = "processStatus";
    private String processId = "241203-3J310-W113";
    private String processDefinitionKey = "a2023chuchai";
    private String tenantId = "trinity";

    @Test
    @DisplayName("流程实例删除")
    public void test001() {
        runtimeService.setVariable(processId,statusKey, "已删除");
        runtimeService.deleteProcessInstance(processId, "测试手动删除");
    }
    ...
    public void testStartProcess() {
        ...
        ProcessInstance holidayRequest = runtimeService.createProcessInstanceBuilder()
                .predefineProcessInstanceId(processId)
    ...
    @Test
    @DisplayName("组长-完成任务")
    public void testCompleteTask011() throws IOException {
        //String userId = "组长1";
        //String userId = "车间主任1";
        //String userId = "财务1";
        String userId = "总经理1";
        ...
        variables.put("result_"+userId, FileUtils.readFileToString(new File("D:\\liji.txt"), Charset.forName("utf-8"))); // 拒绝请假
        // 完成任务
        for (Task task : tasks) {
            System.out.println();
            System.out.println("********* 任务ID = " + task.getId() + " " + task.getName()+"  "+task.getProcessInstanceId());
